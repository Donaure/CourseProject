...
class Runner(object): 
    ...
    def train_model(self, train_loader, model, loss_fn, optimizer, desc='', startseq_idx=0, pad_value=0, p=1):
        running_acc = 0.0
        running_loss = 0.0
        model.train()
    ...
    def train(self, config_file, **kwargs):
        ...
        # print(args["train_args"]["batch_size"])
        # print(args["train_args"]["learning_rate"])
        ...
        k = args['forcing_decay']
        for epoch in range(num_epochs):
            # scheduled sampling
            p = 1  # use gt to  predict next word with probability p
            if args['scheduled_sampling'] == 'linear':
                p = max(0.1, 1 - k * epoch)
            elif args['scheduled_sampling'] == 'exp':
                p = k**epoch
            elif args['scheduled_sampling'] == 'sigmoid':
    ...
    def evaluate(self, config_file, **kwargs):
        ...
        model = Captioner(encoded_image_size=14, encoder_dim=768,
                          attention_dim=args["attention_dim"],
                          embed_dim=args["embedding_dim"],
                          decoder_dim=args["decoder_size"],
