...
import torchvision
from ViTExtractor import ViTExtractor
...
class Encoder(nn.Module):
    ...
    def forward(self, images):
        ...
        with torch.no_grad():
            out = self.extractor.extract_descriptors(
                images,
        ...
        out = out.reshape(
            out.shape[0], self.extractor.num_patches[0], self.extractor.num_patches[1], -1
        ).permute(0, 3, 1, 2)

        # print("descriptor shape: ", out.shape)

        # (batch_size, 768, encoded_image_size, encoded_image_size)
        out = self.adaptive_pool(out)
        # (batch_size, encoded_image_size, encoded_image_size, 768)
        out = out.permute(0, 2, 3, 1)
...
class DecoderWithAttention(nn.Module):
    ...
    def forward(self, encoder_out, encoded_captions, caption_lengths, startseq_idx=0, pad_value=0, p=1):
        """
        Forward propagation.
        ...
        for t in range(max(decode_lengths)):
            ...
            # scheduled sampling
            if t == 0 or torch.rand(1) < p:
                ebds = embeddings[:batch_size_t, t, :]
            else:
                ebds = self.embedding(prev_timestamp_words)[:batch_size_t, 0]

            h, c = self.decode_step(
                torch.cat([ebds, attention_weighted_encoding], dim=1),
            ...
            prev_timestamp_words = preds.argmax(1).unsqueeze(1)
...
class Captioner(nn.Module):
    ...
    def forward(self, images, encoded_captions, caption_lengths, startseq_idx=0, pad_value=0, p=1):
        """
